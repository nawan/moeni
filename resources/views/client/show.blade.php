    ...
    <div class="card-group bg-light mt-10">
        <div class="card-body text-center col-md-6">
            ...
            <div class="card-body bg-white p-5" style="width:100%;max-heigth:600px">
                <img src="{{ asset('storage/' . $client->image) }}" class="rounded img-thumbnail" width="500" style="cursor: pointer;" data-bs-toggle="modal" data-bs-target="#detail-ktp">
            </div>
        </div>

        {{-- modal view show KTP --}}
        <div class="modal fade" data-bs-backdrop="static" data-bs-keyboard="false" id="detail-ktp" tabindex="-1" aria-labelledby="Foto KTP {{ $client->name }}" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered modal-lg">
                <div class="modal-content bg-transparent" style="border: none">
                    ...
                    <div class="modal-body d-flex justify-content-center">
                        <img src="{{ asset('storage/' . $client->image) }}" style="width:100%;max-width:600px">
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="card-body col-md-6">
            ...
            <div class="card-body bg-white p-3" style="width:100%;max-heigth:600px">
                <div class="card-text">
                    ...
                    <p class="fst-italic text-capitalize mb-2">{{ $client->name }}</p>
                    <p class="card-text fw-bold m-0">NIK</p>
                    <p class="fst-italic text-capitalize mb-2">{{ $client->nik }}</p>
                    <p class="card-text fw-bold m-0">No Kontak</p>
                    <p class="fst-italic text-capitalize mb-2">{{ $client->no_kontak }}</p>
                    <p class="card-text fw-bold m-0">Email</p>
                    <p class="fst-italic mb-2">{{ $client->email }}</p>
                    <p class="card-text fw-bold m-0">Alamat</p>
                    <p class="fst-italic text-capitalize mb-2">{{ $client->alamat }}</p>
                </div>
                <div class="d-flex justify-content-end mt-3 gap-2">
                    @php $encryptID = Crypt::encrypt($client->id); @endphp
                    <a href="{{ route('client.edit', $encryptID) }}"class="btn btn-warning m-1" title="Tulis Catatan" data-toggle="tooltip" data-placement="top"><i class="fas fa-edit"></i></a>
                </div>
            </div>
        </div>
